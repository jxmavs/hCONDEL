...
stat_out_file=${outDir}/${outHeader}_stat_out.txt
echo "${outDir}/${outHeader}_stat_out.txt"
...
bedtools intersect -sorted -a ${posFile} -b ${wigDir}/${roadmapID}-${modType}.pval.signal.bedgraph -loj > ${outDir}/${outHeader}_intersected.pval.txt
bedtools intersect -sorted -a ${posFile} -b ${wigDir}/${roadmapID}-${modType}.fc.signal.bedgraph -loj > ${outDir}/${outHeader}_intersected.fc.txt
...
python ${scriptdir}/extractTypeScores_context.py ${outDir}/${outHeader}_intersected.pval.txt ${cutOffVal} ${outDir}/${outHeader}_score_out.pval.txt
python ${scriptdir}/extractTypeScores_context.py ${outDir}/${outHeader}_intersected.fc.txt ${cutOffVal} ${outDir}/${outHeader}_score_out.fc.txt

#echo "python ${scriptdir}/extractTypeScores_context.py ${outDir}/${outHeader}_intersected.txt ${cutOffVal} ${score_out_file}"
#rm -f ${score_out_file}
#process the file and get stats
numSigCoordPval=$(awk '( ($(NF-1)==1)&& ($NF!=-1) ){print}' ${outDir}/${outHeader}_score_out.pval.txt | wc -l)
totalNumCoordPval=$(awk '$NF!=-1{print}' ${outDir}/${outHeader}_score_out.pval.txt | wc -l)
...
#echo ${score_out_file}
totalScorePval=$(cat ${outDir}/${outHeader}_score_out.pval.txt | awk '$4!=-1{ sum+=$4} END {print sum}' )
#echo ${totalScore}
totalLenPval=$(cat ${outDir}/${outHeader}_score_out.pval.txt | awk '$5!=-1{ sum+=$5} END {print sum}' )
#echo ${totalLen}
...
numSigCoordFc=$(awk '( ($(NF-1)==1)&& ($NF!=-1) ){print}' ${outDir}/${outHeader}_score_out.fc.txt | wc -l)
totalNumCoordFc=$(awk '$NF!=-1{print}' ${outDir}/${outHeader}_score_out.fc.txt | wc -l)
#propSigCoord=$(echo ${numSigCoord}/${totalNumCoord} | bc -l)
propSigCoordFc=$(echo -e "${numSigCoordFc}\t${totalNumCoordFc}" | awk -F"\t" '{print $1/$2}')
#echo ${score_out_file}
totalScoreFc=$(cat ${outDir}/${outHeader}_score_out.fc.txt | awk '$4!=-1{ sum+=$4} END {print sum}' )
#echo ${totalScore} 
totalLenFc=$(cat ${outDir}/${outHeader}_score_out.fc.txt | awk '$5!=-1{ sum+=$5} END {print sum}' )
#echo ${totalLen}
avgScoreFc=$(echo -e "${totalScoreFc}\t${totalLenFc}" | awk -F"\t" '{print $1/$2}')
#echo ${avgScore}

echo -e "${avgScorePval}\t${propSigCoordPval}\t${totalNumCoordPval}\t${avgScoreFc}\t${propSigCoordFc}\t${totalNumCoordFc}\t${roadmapID}"
echo -e "${avgScorePval}\t${propSigCoordPval}\t${totalNumCoordPval}\t${avgScoreFc}\t${propSigCoordFc}\t${totalNumCoordFc}\t${roadmapID}"  > ${stat_out_file}

rm -f ${outDir}/${outHeader}_intersected.pval.txt
rm -f ${outDir}/${outHeader}_intersected.fc.txt
rm -f ${outDir}/${outHeader}_score_out.pval.txt
rm -f ${outDir}/${outHeader}_score_out.fc.txt
