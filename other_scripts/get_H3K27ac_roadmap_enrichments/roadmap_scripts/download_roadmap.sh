...
rm -f ${wrkdir}/all_runs_nonimputed_${modType}.txt

while read -a array
do
	roadmapID=${array[0]}
    echo "bash download_roadmap_single_data.sh ${roadmapID} ${modType} ${outDir}" >> ${wrkdir}/all_runs_nonimputed_${modType}.txt
...
totalMem=5G
totalNumTasks=$(wc -l ${wrkdir}/all_runs_nonimputed_${modType}.txt  | awk '{print $1}')   
totalTime=20:00:00
sed "s/#$ -t 1-totalNumTasks/#$ -t 1-${totalNumTasks}/g; s/#$ -l h_vmem=totalMem/#$ -l h_vmem=${totalMem}/g; s/#$ -l h_rt=totalTime/#$ -l h_rt=${totalTime}/g" ${del_dir}/runTasksEval.sh > ${wrkdir}/download_nonimputed_${modType}_all.sh
rm -f ${wrkdir}/download_nonimputed_${modType}_all.log
qsub -o ${wrkdir}/download_nonimputed_${modType}_all.log ${wrkdir}/download_nonimputed_${modType}_all.sh ${wrkdir}/all_runs_nonimputed_${modType}.txt
