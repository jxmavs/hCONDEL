...
do
    ...
    if [[ ${iter} -eq 1 ]];
        then
            ...
            awk '{print $1"\t"$2}' ${fileName} >  ${temp_path}/temp_count_barcode_file.txt
            
        else
            paste <(cat ${temp_path}/temp_count_barcode_file.txt) <(awk '{print $2}' ${fileName})  > ${temp_path}/temp_count_barcode_file_augment.txt
            #echo "paste <(cat ${temp_path}/temp_count_barcode_file.txt) <(awk 'NR>1{print $5}' ${fileName}) "
            mv ${temp_path}/temp_count_barcode_file_augment.txt ${temp_path}/temp_count_barcode_file.txt
    fi
...
echo -e ${headerString} > ${file_path}/${header_name}_barcode.txt
cat ${temp_path}/temp_count_barcode_file.txt >> ${file_path}/${header_name}_barcode.txt
