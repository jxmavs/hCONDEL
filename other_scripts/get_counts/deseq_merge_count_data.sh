...
do
    ...
    if [[ ${iter} -eq 1 ]];
        then
            ...
            awk '{print $1"\t"$3}' ${fileName} >  ${temp_path}/temp_count_file.txt
            
        else
            paste <(cat ${temp_path}/temp_count_file.txt) <(awk '{print $3}' ${fileName})  > ${temp_path}/temp_count_file_augment.txt
            #echo "paste <(cat ${temp_path}/temp_count_file.txt) <(awk 'NR>1{print $5}' ${fileName}) "
            mv ${temp_path}/temp_count_file_augment.txt ${temp_path}/temp_count_file.txt
   fi
...
cat ${temp_path}/temp_count_file.txt >> ${file_path}/${header_name}_deseq.txt
