...
do
    echo "bash ${roadmap_script_dir}/gen_null_seq_iter_rand_cons_background_v2_alternate_v2.sh ${permInd} ${out_header} ${outdir} ${cons_seq_comp_stat_file} ${orig_seq_comp_stat_file} ${lengthBoundPct} ${misMatchBoundPct} ${GCBoundPct} ${logOddsBoundPct} ${sampleNullSeqMaxTrys} ${sampleNullSeqConsDevPct}" >> ${wrkdir}/all_null_seq_${out_header}.txt
